"""CLI entry point — read SpecQualityInput from a path, write QualityVerdict to stdout.

FT-132 / ADR-073 worker contract:

* Invocation: ``python -m spec_quality --bundle <path-to-input-json>`` or
  ``python -m spec_quality --stdin``
...
* Exit codes for infrastructure faults per FT-132's error table:
...
def _parse_args(argv: Sequence[str] | None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="spec-quality",
        description=(
            "Stateless Python worker for decision-cli's spec-quality role. "
            "Bundle in, QualityVerdict artifact out (ADR-073, FT-132)."
        ),
    )
    ...
    src.add_argument(
        ...
        type=Path,
        default=None,
        help="Path to a SpecQualityInput JSON document (FT-132 default invocation).",
    )
    src.add_argument(
        "--stdin",
        action="store_true",
...
def _read_bundle_source(args: argparse.Namespace) -> tuple[str | None, str | None]:
    ...
    raw = sys.stdin.read()
    if not raw.strip():
        return None, "empty stdin — pass --bundle <path> or pipe a SpecQualityInput JSON"
    return raw, None
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if read_err is not None:
        sys.stderr.write(f"spec-quality: bundle parse error: {read_err}\n")
        return 2

    try:
        bundle = SpecQualityInput.model_validate_json(raw)
    except ValidationError as exc:
        sys.stderr.write(f"spec-quality: bundle parse error: {exc}\n")
        return 2

    if not _BUNDLE_HASH_RE.match(bundle.bundle_hash):
        sys.stderr.write(
            "spec-quality: bundle parse error: bundle_hash is malformed "
    ...
    try:
        ...
        sys.stderr.write(f"spec-quality: bundle_hash mismatch: {exc}\n")
        return 5
    except WorkerError as exc:
        message = str(exc)
        if "failed validation after retry" in message:
            sys.stderr.write(f"spec-quality: schema validation failed: {message}\n")
            return 4
        sys.stderr.write(f"spec-quality: model call failed: {message}\n")
        return 3
    except Exception as exc:  # noqa: BLE001 - any unexpected failure
        sys.stderr.write(f"spec-quality: model call failed: {exc}\n")
        return 3
