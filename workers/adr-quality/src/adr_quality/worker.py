"""Dispatches the adr-quality role via ModelRouter, returning a QualityVerdict."""
...
def run_judge(
    ...
    * Otherwise → a router is built from ``bundle.endpoint``. This is the
    ...
    # Bundle-hash echo check — FT-133 / ADR-073.
...
def _build_router_for_bundle(bundle: AdrQualityInput) -> Any:
...
def _try_parse_verdict(
    raw: str, bundle: AdrQualityInput
...
def _stub_caller(system: str, user: str, model_id: str, max_tokens: int) -> tuple[str, int, int]:
    """Deterministic stub used when ADR_QUALITY_STUB=1.
    ...
    feature_id = _find_feature_id_in_prompt(user)
    proposal_iri = (
        _find_proposal_iri_in_prompt(user) or f"urn:adr-proposal:{feature_id}"
    )
    gap_iri = _find_against_gap_iri_in_prompt(user) or f"urn:preflight-gap:{feature_id}"
    spec_iri = _find_against_spec_iri_in_prompt(user) or f"urn:feature-spec:{feature_id}"
    payload = {
        "verdict": "approved",
        "rationale": (
            "stub mode (ADR_QUALITY_STUB=1): worker returns a synthetic "
            "approved verdict so smoke-tests do not depend on a live model session."
        ),
        "judges": proposal_iri,
        "against": [gap_iri, spec_iri],
        "violates": [],
...
def _find_feature_id_in_prompt(user: str) -> str:
    """Heuristic — extract the feature_id line from the rendered prompt."""
    marker = "**Feature**:"
    for line in user.splitlines():
        if marker in line:
            return line.split(marker, 1)[1].strip()
...
def _find_proposal_iri_in_prompt(user: str) -> str:
    """Heuristic — extract the AdrProposal IRI line from the rendered prompt."""
    marker = "**AdrProposal IRI**:"
    for line in user.splitlines():
        if marker in line:
...
def _find_against_gap_iri_in_prompt(user: str) -> str:
    """Heuristic — extract the preflight_gap IRI hint embedded in the task instructions."""
    marker = "the preflight_gap IRI"
    for line in user.splitlines():
        if marker in line:
            # The line looks like: "- **against**: list of source-of-truth IRIs — the preflight_gap IRI ('urn:...') AND the feature_spec IRI ('urn:...')"
            return _between_first_single_quotes(line)
    return ""


def _find_against_spec_iri_in_prompt(user: str) -> str:
    """Heuristic — extract the feature_spec IRI hint embedded in the task instructions."""
    marker = "the feature_spec IRI"
    for line in user.splitlines():
        if marker in line:
            return _between_last_single_quotes(line)
    return ""


def _between_first_single_quotes(line: str) -> str:
    """Return the substring between the FIRST pair of single quotes in `line`."""
    start = line.find("'")
    if start < 0:
        return ""
...
def _between_last_single_quotes(line: str) -> str:
    """Return the substring between the LAST pair of single quotes in `line`."""
    end = line.rfind("'")
    if end < 0:
        return ""
