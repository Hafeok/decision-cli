"""CLI entry point — read TcAuthorInput from a path, write TcProposal to stdout.

FT-126 / ADR-073 worker contract:

* Invocation: ``python -m tc_author --bundle <path-to-input-json>`` or
  ``python -m tc_author --stdin``
* Exit 0 iff a Pydantic-validated TcProposal was emitted on stdout
  (regardless of `kind`: sufficient, augment, new are all valid outcomes).
* Exit codes for infrastructure faults per FT-126's error table:
    ...
    - 5: TcProposal.bundle_hash ≠ input bundle_hash (protocol bug)
...
def _parse_args(argv: Sequence[str] | None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="tc-author",
        description=(
            "Stateless Python worker for decision-cli's tc-author role. "
            "Bundle in, TcProposal artifact out (ADR-073, FT-126)."
        ),
    )
    ...
    src.add_argument(
        ...
        type=Path,
        default=None,
        help="Path to a TcAuthorInput JSON document (FT-126 default invocation).",
    )
    src.add_argument(
        "--stdin",
        action="store_true",
...
def _read_bundle_source(args: argparse.Namespace) -> tuple[str | None, str | None]:
    ...
    raw = sys.stdin.read()
    if not raw.strip():
        return None, "empty stdin — pass --bundle <path> or pipe a TcAuthorInput JSON"
    return raw, None
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if read_err is not None:
        sys.stderr.write(f"tc-author: bundle parse error: {read_err}\n")
        return 2

    try:
        bundle = TcAuthorInput.model_validate_json(raw)
    except ValidationError as exc:
        sys.stderr.write(f"tc-author: bundle parse error: {exc}\n")
        return 2

    if not _BUNDLE_HASH_RE.match(bundle.bundle_hash):
        sys.stderr.write(
            "tc-author: bundle parse error: bundle_hash is malformed "
    ...
    try:
        ...
        sys.stderr.write(f"tc-author: bundle_hash mismatch: {exc}\n")
        return 5
    except WorkerError as exc:
        message = str(exc)
        if "failed validation after retry" in message:
            sys.stderr.write(f"tc-author: schema validation failed: {message}\n")
            return 4
        sys.stderr.write(f"tc-author: model call failed: {message}\n")
        return 3
    except Exception as exc:  # noqa: BLE001 - any unexpected failure
        sys.stderr.write(f"tc-author: model call failed: {exc}\n")
        return 3
