"""Dispatches the tc-quality role via ModelRouter, returning a QualityVerdict."""
...
def run_judge(
    ...
    * Otherwise → a router is built from ``bundle.endpoint``. This is the
    ...
    # Bundle-hash echo check — FT-127 / ADR-073.
...
def _build_router_for_bundle(bundle: TcQualityInput) -> Any:
...
def _stub_caller(system: str, user: str, model_id: str, max_tokens: int) -> tuple[str, int, int]:
    """Deterministic stub used when TC_QUALITY_STUB=1.
    ...
    # Extract feature_id from the user prompt
    feature_id = _find_feature_id_in_prompt(user)
    payload = {
        "verdict": "approved",
        "rationale": (
            "stub mode (TC_QUALITY_STUB=1): worker returns a synthetic "
            "approved verdict so smoke-tests do not depend on a live model session."
        ),
        "judges": f"urn:tc-proposal:{feature_id}",
        "against": [f"urn:feature:{feature_id}"],
        "violates": [],
