...
class QualityVerdict(BaseModel):
    """A tc-quality verdict judging a TcProposal's fitness (ADR-074, FT-127).

    Structurally identical to VerificationVerdict (ADR-018) except for:
    - `judges` (IRI of the artifact under judgment) instead of `verifies`
    - `against` (list of source-of-truth IRIs) instead of a single target
    ...
    judges: str = Field(
        ...,
        description="IRI of the TcProposal artifact under judgment (ADR-074 polymorphism).",
    )
    against: list[str] = Field(
        ...,
        min_items=1,
        description="Source-of-truth IRIs the artifact is judged against (feature_spec for TC quality).",
    )
