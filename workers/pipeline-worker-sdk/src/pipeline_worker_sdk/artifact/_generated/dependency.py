"""Typed builder for dec:Dependency artifacts."""
...
class DependencyBuilder(BuilderBase):
    """Builder for emitting dec:Dependency artifacts.
    ...
    TARGET_CLASS_LOCAL: str = 'Dependency'
    SOURCE_SHAPE: str = 'workers/_shared/shapes/dependency.ttl'
    ACCEPTS_BOUNDARY: bool = True
    MOTIVATIONAL: tuple[MotivationalDescriptor, ...] = (
        MotivationalDescriptor(
            predicate_local='requiredBy',
            predicate_iri='https://decision-cli.dev/ns#requiredBy',
            target_class_local='ADR',
            target_class_iri='https://decision-cli.dev/ns#ADR',
        ),
    )

    P_requiredBy: str = 'https://decision-cli.dev/ns#requiredBy'

    def __init__(self, iri: str) -> None:
        super().__init__(iri)
        self._requiredBy: list[str] = []

    def add_requiredBy(self, target_iri: str) -> "DependencyBuilder":
        """Add a motivational ``requiredBy`` edge (one of the
        sh:or alternatives declared in the per-type shape).
        """
        self._requiredBy.append(target_iri)
        return self
    ...
    def _motivational_state(self) -> dict[str, bool]:
        ...
        return {
            'requiredBy': bool(self._requiredBy),
        }
    ...
    def _type_triples(self) -> list[tuple[str, str, str]]:
        ...
        for v in self._requiredBy:
            triples.append((self.iri, self.P_requiredBy, v))
        return triples
