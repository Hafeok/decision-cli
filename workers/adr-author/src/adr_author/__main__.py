"""CLI entry point — read AdrAuthorInput from a path, write AdrProposal to stdout.

FT-130 / ADR-073 worker contract:

* Invocation: ``python -m adr_author --bundle <path-to-input-json>`` or
  ``python -m adr_author --stdin``
* Exit 0 iff a Pydantic-validated AdrProposal was emitted on stdout
  (regardless of `kind`: new, acknowledgement, or gap are all valid).
* Exit codes for infrastructure faults per FT-130's error table:
    ...
    - 5: AdrProposal.bundle_hash ≠ input bundle_hash (protocol bug)
...
def _parse_args(argv: Sequence[str] | None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="adr-author",
        description=(
            "Stateless Python worker for decision-cli's adr-author role. "
            "Bundle in, AdrProposal artifact out (ADR-073, FT-130)."
        ),
    )
    ...
    src.add_argument(
        ...
        type=Path,
        default=None,
        help="Path to an AdrAuthorInput JSON document (FT-130 default invocation).",
    )
    src.add_argument(
        "--stdin",
        action="store_true",
...
def _read_bundle_source(args: argparse.Namespace) -> tuple[str | None, str | None]:
    ...
    raw = sys.stdin.read()
    if not raw.strip():
        return None, "empty stdin — pass --bundle <path> or pipe an AdrAuthorInput JSON"
    return raw, None
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if read_err is not None:
        sys.stderr.write(f"adr-author: bundle parse error: {read_err}\n")
        return 2

    try:
        bundle = AdrAuthorInput.model_validate_json(raw)
    except ValidationError as exc:
        sys.stderr.write(f"adr-author: bundle parse error: {exc}\n")
        return 2

    if not _BUNDLE_HASH_RE.match(bundle.bundle_hash):
        sys.stderr.write(
            "adr-author: bundle parse error: bundle_hash is malformed "
    ...
    try:
        ...
        sys.stderr.write(f"adr-author: bundle_hash mismatch: {exc}\n")
        return 5
    except WorkerError as exc:
        message = str(exc)
        if "failed validation after retry" in message:
            sys.stderr.write(f"adr-author: schema validation failed: {message}\n")
            return 4
        sys.stderr.write(f"adr-author: model call failed: {message}\n")
        return 3
    except Exception as exc:  # noqa: BLE001 - any unexpected failure
        sys.stderr.write(f"adr-author: model call failed: {exc}\n")
        return 3
