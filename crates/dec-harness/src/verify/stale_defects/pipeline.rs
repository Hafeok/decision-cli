...
use dec_graph::ontology::verification_result::{EvidenceProjection, VerificationGraphResult};
...
pub fn retract_stale_defects(
    ...
    let scope =
        dec_graph::scope::ActiveScope::load(workdir).map_err(|e| format!("loading scope: {e}"))?;
    let stream_iri =
    ...
    ?ev dec:tc ?tc ;
        ...
        dec:fromStep ?fromStep .
            ...
            return Ok(VerificationGraphResult {
                ...
                verdict: dec_graph::ontology::verdict::Verdict::parse(&verdict_str)
