...
use crate::feedback::lifecycle::LifecycleState;
use crate::feedback::transition::{apply as apply_transition, ApplyError};
use dec_graph::stream_writer::StreamWriter;
use dec_ontology::vocab::{addressing_artifact, orchestration_graph, routed_at};
...
pub fn mark_addressed(
    ...
    let prior = match crate::feedback::transition::read_prior_state(store, &fb_node) {
        Ok(state) => state,
    ...
    if matches!(cur, LifecycleState::Routed) {
        let received_evidence = vec![Quad::new(
            fb_node.clone(),
            dec_ontology::vocab::receiving_session().into_owned(),
            session_iri.clone(),
            g.clone(),
        )];
        apply_transition(
...
pub fn mark_batch_addressed(
    ...
    let dump = dec_graph::store::orchestration_dump_path(workdir);
    let store = dec_graph::store::load_store_from_dump(&dump).map_err(|e| {
        AddressWalkError::Transition(ApplyError::Store(format!("loading store: {e:#}")))
    })?;
    let store = Arc::new(store);
    let scope = dec_graph::scope::ActiveScope::load(workdir).map_err(|e| {
    ...
    dec_graph::store::persist_store(&store, &dump).map_err(|e| {
