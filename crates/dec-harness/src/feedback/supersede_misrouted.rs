...
use crate::feedback::read::list_by_class;
use crate::feedback::transition::apply as apply_transition;
...
fn supersede_with_role_twin(
    ...
    let evidence = vec![Quad::new(
        old.iri.clone(),
        NamedNodeRef::new_unchecked(dec_ontology::vocab::IRI_DEC_SUPERSEDED_BY).into_owned(),
        twin_iri.clone(),
        g.clone(),
    )];
