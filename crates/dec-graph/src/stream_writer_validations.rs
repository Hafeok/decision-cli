...
use crate::bundle::validate_quads as validate_bundle_quads;
use crate::ontology::capability::validate_quads as validate_capability_quads;
use crate::ontology::catalog::validate_quads_with_store as validate_catalog_quads_with_store;
use crate::ontology::coverage_waiver::validate_quads as validate_waiver_quads;
use crate::ontology::role_binding::validate_quads as validate_role_binding_quads;
use crate::ontology::verdict::validate_quads as validate_verdict_quads;
...
/// SHACL-validate every `dec:Archetype` subject present in `quads`
/// (FT-147 / ADR-082 / ADR-084 §1 — E102 fires here).
pub(super) fn validate_archetypes(quads: &[Quad]) -> Result<()> {
    validate_archetype_quads(quads).map_err(|err| {
        anyhow!(
            "SHACL violation: archetype mutation refused\n{}",
            err.report
        )
    })
}
