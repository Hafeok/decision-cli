...
//!
//! When [`Validator::validate`](crate::graph::shacl::Validator::validate)
...
fn evidence_quad(
    ...
    Quad::new(
        feedback_iri.clone(),
        dec_ontology::vocab::evidence().into_owned(),
...
mod tests {
    ...
    fn feedback_quads_cover_required_shape() {
        ...
        assert!(predicates.contains(&dec_ontology::vocab::IRI_DEC_FEEDBACK_CLASS));
        assert!(predicates.contains(&dec_ontology::vocab::IRI_DEC_LIFECYCLE_STATE));
        assert!(predicates.contains(&dec_ontology::vocab::IRI_DEC_TARGET_ROLE));
        assert!(predicates.contains(&dec_ontology::vocab::IRI_DEC_EVIDENCE));
        assert!(predicates.contains(&dec_ontology::vocab::IRI_DEC_SOURCE_SESSION));
        assert!(predicates.contains(&dec_ontology::vocab::IRI_DEC_SEVERITY));
        assert!(predicates.contains(&dec_ontology::vocab::IRI_DEC_IN_STREAM));
    }
}
