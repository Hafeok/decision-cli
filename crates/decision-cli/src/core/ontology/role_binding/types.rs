...
impl RoleBinding {
    ...
    fn binding_quads(&self, subject: &NamedNode, g: &GraphName) -> Vec<Quad> {
        ...
        let mut out = vec![
            ...
            named_quad(subject, default_capability_pred(), &self.default_capability, g),
            typed_literal_quad(
                subject,
    ...
    fn list_quads(&self, subject: &NamedNode, g: &GraphName) -> Vec<Quad> {
        ...
        for (k, _step) in self.escalation_steps.iter().enumerate() {
            ...
            out.push(named_quad(&node, NamedNodeRef::new_unchecked(RDF_REST), &rest, g));
        }
        out
    }
    ...
    fn step_quads(&self, i: usize, step: &EscalationStep, g: &GraphName) -> Vec<Quad> {
        ...
        let rdf_type = NamedNodeRef::new_unchecked(RDF_TYPE);
        let mut out = vec![
            Quad::new(step_iri.clone(), rdf_type, escalation_step_class(), g.clone()),
        ...
        for (j, t) in step.triggers.iter().enumerate() {
            ...
            out.push(literal_quad(&trigger_iri, trigger_signal_pred(), t.as_str(), g));
