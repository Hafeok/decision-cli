...
use crate::core::ontology::boundary_artifact::{external_origin_pred, initial_request_class};
use crate::core::vocab::{
    ...
    submission_lifecycle_state_pred, worker_image_submission_class, IRI_DEC_WORKER_IMAGE_SUBMISSION,
    IRI_DEC_WORKER_IMAGE_SUBMISSION_PREFIX, SUBMISSION_STATE_ADMITTED, SUBMISSION_STATE_RECEIVED,
...
pub(super) const WORKER_IMAGE_SUBMISSION_CLASS_IRI: &str = IRI_DEC_WORKER_IMAGE_SUBMISSION;
...
impl WorkerImageSubmission {
    ...
    fn header_quads(&self, subject: &NamedNode, g: &GraphName) -> Vec<Quad> {
        ...
        vec![
            ...
            Quad::new(subject.clone(), rdf_type, initial_request_class(), g.clone()),
        ]
    }
