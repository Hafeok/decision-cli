...
use crate::core::ontology::{EXTERNAL_ORIGIN_PROP, IS_MIGRATION_BACKFILL_PROP};
...
fn duplicate_external_origin_is_rejected() {
    ...
    assert!(err.report.contains("expected exactly one"), "{}", err.report);
}
...
fn well_formed_is_migration_backfill_validates() {
    ...
    validate_migration_backfill(&quads, &a)
        .expect("dec:isMigrationBackfill true must validate");
}
