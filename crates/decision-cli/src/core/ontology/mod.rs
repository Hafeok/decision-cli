//! Store-facing facade over the embedded base ontology (FT-006 / ADR-007).
//!
//! The pure domain layer — typed artifact submodules, IRI constants, and
//! the embedded Turtle assets — lives in [`dec_ontology::ontology`]
//! (ADR-086) and is re-exported here wholesale so existing
//! `crate::core::ontology::…` paths keep working. This module keeps the
//! parts that need a store: [`OntologyHandle`] (parse + cache + FT-006
//! invariant checks) and the store-backed read/validation glue
//! submodules declared below, which shadow their pure counterparts and
//! re-export both halves.
//!
...
use per_type_invariants::invariant_per_type_shape_files_present;
...
impl OntologyHandle {
    ...
    fn parse() -> Result<Self, OntologyError> {
        ...
        load_turtle_into_graph(&store, MECHANICAL_PROVENANCE_SHAPES_TTL, SHAPES_GRAPH_IRI)?;
