...
use super::{
    binding_admin,
    binding_admin::{ActivateBindingArgs, DeactivateBindingArgs, ListBindingsArgs},
    bootstrap_catalog,
    bootstrap_catalog::BootstrapCatalogArgs,
    check_goal,
    check_goal::CheckGoalArgs,
    doctor,
    doctor::DoctorArgs,
    drive,
    drive::DriveCmd,
    ...
    init::InitArgs,
    internal_dispatch,
    internal_dispatch::InternalDispatchCmd,
    loop_cmd,
    loop_cmd::LoopCmd,
    mcp,
    mcp::McpCmd,
    ...
    query,
    query::QueryCmd,
    retract_orphan_defects,
    retract_orphan_defects::RetractOrphanDefectsArgs,
    retract_stale_defects,
    retract_stale_defects::RetractStaleDefectsArgs,
    seed_ft101_catalog,
    seed_ft101_catalog::SeedFt101CatalogArgs,
    session,
    session::SessionCmd,
    sparql,
    sparql::SparqlArgs,
    status, supersede_graph,
    supersede_graph::SupersedeGraphArgs,
    supersede_misrouted,
    supersede_misrouted::SupersedeMisroutedArgs,
    verify,
    verify::VerifyCmd,
    workers,
    workers::WorkersCmd,
...
pub enum Command {
    ...
    #[command(subcommand)]
    Loop(LoopCmd),
    /// Pluggable artifact+goal orchestrator and drive history viewer (FT-110, FT-113).
    #[command(subcommand)]
    Drive(DriveCmd),
...
pub fn dispatch(workdir: &Path, command: Command) -> ExitCode {
    match command {
        ...
        Command::Drive(cmd) => drive::run(workdir, cmd),
