...
//!
//! Spec: `.product/tests/TC-061-dec-verify-bench-list-returns-ordered-envs-with-safe.md`
...
use decision_cli::verify_bench_list::{self, BenchListRequest, EnvSummary, OutputFormat};
...
/// Author three additional envs on top of the seed (`BNCH-001-ephemeral-cli`):
///   * `BNCH-002` — isolated, ephemeral-tempdir (extra ops to vary the row).
...
fn new_env(_workdir: &Path, req: BenchNewRequest) {
    verify_bench_new::run(&req).expect("env new must succeed for fixture");
}

fn run_list(workdir: &Path, req: BenchListRequest) -> Vec<EnvSummary> {
    let mut req = req;
    if req.workdir.is_none() {
        req.workdir = Some(workdir.to_path_buf());
    }
    let resp = verify_bench_list::run(&req).expect("env list ok");
    resp.envs
}
...
fn mcp_invoke(
    workdir: &Path,
    args: Value,
) -> Result<verify_bench_list::BenchListResponse, HandlerError> {
    let mut obj = args;
    obj.as_object_mut()
        .expect("args is object")
        .insert("workdir".to_string(), json!(workdir.to_string_lossy()));
    let req = Request::new(verify_bench_list::TOOL_NAME, obj);
    let parsed = verify_bench_list::parse_request(&req)?;
    verify_bench_list::run(&parsed)
}
...
fn ac1_empty_store_returns_single_seed_env() {
    ...
    let envs = run_list(tmp.path(), BenchListRequest::default());
    assert_eq!(envs.len(), 1, "exactly the seeded env should be present");
    assert!(
        envs[0].id.starts_with("BNCH-001"),
        "expected BNCH-001 prefix, got {:?}",
        envs[0].id
    );
    assert_eq!(envs[0].safety_class, "isolated");
    assert_eq!(envs[0].bench_type, "ephemeral-tempdir");

    // --format json over the same store returns a one-element array.
    let resp = verify_bench_list::BenchListResponse { envs };
    let json = verify_bench_list::render_json(&resp);
...
fn ac2_envs_returned_in_ascending_numeric_order() {
    ...
    // Order is BNCH-001-ephemeral-cli, BNCH-002, ENV-003, ENV-004
    // (the seed env's suffix sorts after pure BNCH-001 — but here only
    // BNCH-001-ephemeral-cli exists at the "001" slot, so ascending by
    // numeric tail then string puts it first).
    assert_eq!(ids.len(), 4);
    assert!(ids[0].starts_with("BNCH-001"));
    assert_eq!(ids[1], "BNCH-002");
...
fn ac3_filter_by_safety_class() {
    ...
    // Seed (ephemeral-cli) is isolated; BNCH-002 is isolated.
    assert!(envs.iter().all(|e| e.safety_class == "isolated"));
    assert!(
        envs.iter().any(|e| e.id == "BNCH-002"),
        "isolated set must include BNCH-002"
    );
    assert!(
...
fn ac4_filter_by_env_type() {
    ...
    seed_three_envs(tmp.path());
    let req = BenchListRequest {
        bench_type: Some("remote-http".to_string()),
        ..Default::default()
    };
    let envs = run_list(tmp.path(), req);
    assert!(envs.iter().all(|e| e.bench_type == "remote-http"));
    let ids: Vec<&str> = envs.iter().map(|e| e.id.as_str()).collect();
...
fn ac5_combined_filters_are_conjunctive() {
    ...
    assert_eq!(envs[0].bench_type, "remote-http");
}
...
fn ac6_json_shape_omits_absent_optional_fields() {
    ...
    let envs = run_list(tmp.path(), BenchListRequest::default());
    let resp = verify_bench_list::BenchListResponse { envs };
    let s = verify_bench_list::render_json(&resp);
    ...
    for entry in arr {
        ...
        assert!(obj.contains_key("bench_type"));
        assert!(obj.contains_key("safety_class"));
        assert!(obj.contains_key("allowed_ops"));
    }
    // BNCH-002 has no endpoint / setup / teardown — those keys must be absent.
    let env002 = arr
        .iter()
        .find(|v| v["id"] == "BNCH-002")
        .expect("BNCH-002 in JSON output");
...
fn ac7_mcp_parity_with_cli_json_output() {
    ...
    seed_three_envs(tmp.path());
    // CLI path.
    let cli_envs = run_list(tmp.path(), BenchListRequest::default());
    let cli_resp = verify_bench_list::BenchListResponse {
        envs: cli_envs.clone(),
    };
    let cli_json: Value =
        serde_json::from_str(&verify_bench_list::render_json(&cli_resp)).expect("cli json");
...
fn ac8_invalid_safety_class_returns_invalid_argument() {
    ...
    let err = verify_bench_list::run(&req).expect_err("yolo must fail");
    match err {
...
fn binary_cli_table_format_smoke() {
    ...
    assert!(
        stdout.contains("BNCH-001"),
        "expected BNCH-001 in output: {stdout}"
    );
}
