...
fn ontology_version_bumped_to_0_5_0() {
    use decision_cli::OntologyHandle;
    let h = OntologyHandle::load().expect("load ontology");
    // Bumped to 0.7.0 by FT-069 (mechanical-provenance SHACL fragment);
    // the earlier 0.5.0 contract remains in name because no downstream
    // consumer pinned to a specific string value.
    assert_eq!(h.version(), "0.7.0");
}
