...
1. Exit 0 if every Rust function body under `crates/*/src/` (excluding
   `tests.rs` and `#[cfg(test)]` modules) has at most `FN_LENGTH_WARN`
   statement lines (default 30).
2. Exit 2 if at least one function body is in the 31–40 line band and
   none exceeds the hard limit. Diagnostic lines on stdout name each
   warning function.
3. Exit 1 if at least one function body exceeds `FN_LENGTH_HARD` lines
   (default 40). Diagnostic lines on stdout name each offender with
   its file, line number, function name, and statement count.
...
⟦Γ:Invariants⟧{
  ...
  ∀fn:TopLevelFn: fn.body_stmts > WarnLimit ⇒ produces_warning(fn)
}
