...
The concrete system this architecture targets is **decision-cli** (the orchestration system, binary name `dec`), **product-cli** (the Engineering process system, which already exists), and **oxi-events** (a reusable event substrate extracted as a separate Rust crate). Slice 1 bounds for that work are in a companion document.
...
- **Dagger.io as plug-in substrate**: Dagger's primitives are containerized functions composed into pipelines that run to completion. Our process systems own persistent graph state and react to events continuously. Dagger remains a valid choice *inside* action workers (container-shaped actions: deployers, test runners) but does not substrate the plug-in interface itself.
...
- **Pure execution against external systems** (deployer, test runner, email sender, ticket creator): whatever has the best SDK for the target system. Dagger fits naturally inside container-shaped actions.
...
**decision-cli** — the orchestration system, installed as the `dec` binary. Invokes agents, records sessions, routes artifacts between roles, manages model bindings, manages policy, runs the event substrate, surfaces work to humans for checkpoints, improves itself via the meta-loop. *Does not own engineering artifact knowledge* — calls product-cli for what product-cli already knows.
...
decision-cli (binary `dec`) follows the single-command pattern of `az`, `gcloud`, `kubectl`. Every command is `dec <namespace> <verb> [args]`, with namespaces mapping directly to DDD entities:

- `dec drive ship FT-007` — goal-driven dispatch (the orchestrator plans the role chain to reach a value action)
- `dec dispatch role implementer FT-007` — manual single-role dispatch (power-user escape, debugging, replay)
- `dec watch monitor --env production` — standing role
- `dec schedule pattern-detector --interval 1h` — periodic role
- `dec product feature new FT-007` — engineering artifact authoring (folds in once product-cli is absorbed)
- `dec events tail`, `dec session show ses-abc123`, `dec goal show FT-007` — inspection
- `dec role list`, `dec model list`, `dec policy show` — catalog inspection
- `dec checkpoint pending`, `dec checkpoint approve ses-abc123` — human-in-loop
...
Eventually product-cli folds into decision-cli's workspace as a `product-core` crate, with `dec product …` becoming the unified entry point. That convergence is a release decision, not an architectural one — made when distribution simplification outweighs the cost of coupled release cadence.
...
- **When to fold product-cli into decision-cli's workspace.** Currently subprocess integration in slice 1. Convergence point is when product-cli's API has stabilized — around slice 4-6, once the meta-loop is operational.
